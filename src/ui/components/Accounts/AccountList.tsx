...
export const AccountList: FC = () => {
    ...
    return <>
        ...
        <div
            ...
            {ownUsers.map(user => <DragOpener containerType="Konten" key={user.publicKey} config={{
                type: "component",
                componentType: "Konto",
                ...
                <div
                    onClick={() => !layout && setSelected(selected === user.publicKey ? null : user.publicKey)}
            ...
            {otherUsers.map(user => <DragOpener containerType="Konten" key={user.publicKey} config={{
                type: "component",
                componentType: "Konto",
                componentState: user.publicKey,
                title: user.name
        ...
        {selected && <div className="flex-1 border-t border-gray-200 flex flex-col bg-white min-h-0 overflow-auto">
            <AccountView close={() => setSelected(null)} props={selected}/>
        </div>}
    </>
}